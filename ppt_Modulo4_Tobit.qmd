...
> Nota: La función de **verosimilitud** depende explícitamente de la desviación estándar de los errores. El modelo utiliza la dispersión de los errores para calcular la densidad cuando no hay censura y la probabilidad cuando hay.
...
#### Sección 1: Modelo Tobit: Interpretación de coeficientes {.smaller}

El coeficiente  $\beta_i$ si observamos $y_i^*$  indica:

> El cambio esperado en el ¨**valor latente** $y_i^*$ ante un aumento de una unidad en $x_{j}$, manteniendo constantes las demás variables.
...
- En la realidad no lo observamos.

- Por tanto, debido a la censura, el cambio esperado observado  sobre $y$  es:


$$
\frac{\partial E[y_i \mid x_i]}{\partial x_{ij}}
= \beta_j \cdot \Phi\left(\frac{\mathbf{x_i} \mathbf{\beta} }{\sigma}\right)
$$

- Donde $\Phi$ es la CDF normal.  


- Este efecto es **menor en magnitud** que $\beta_j$ porque sólo aplica a quienes no están censurados.

#### Sección 1: Modelo Tobit: Interpretación de coeficientes {.smaller}

- El factor de ajuste estimado para una $\mathbf{x}$ dada será la probabilidad estimada de observar un valor no censurado:


$$\hat{P}(y_i > 0 \mid \mathbf{x})
= \Phi\left(\frac{\mathbf{x}\hat\beta}{\hat\sigma}\right)$$
...
- Este factor de escala tiene una interpretación interesante: 

Si $\Phi\left(\frac{\mathbf{x}\hat\beta}{\hat\sigma}\right) \approx 1$:

> Prácticamente no existe censura y el Tobit se aproxima a OLS

Si $\Phi\left(\frac{\mathbf{x}\hat\beta}{\hat\sigma}\right) <  1$:

> El efecto marginal es menor que el coeficiente $\beta_j$ porque parte de la muestra permanece en el punto de censura.


#### Sección 1: Modelo Tobit: Interpretación de coeficientes {.smaller}


El efecto marginal de $x_{j}$  **en la probabilidad de no estar censurado** será:

$$
\frac{\partial P(y_i > 0 \mid x_i)}{\partial x_{ij}}
= 
\phi\left(\frac{x_i\beta}{\sigma}\right)\frac{\beta_j}{\sigma}
$$
...
$$hours_i = \beta_0 + \beta_1 \cdot age_i + \beta_2 \cdot educ_i + \beta_3 \cdot exper_i + \beta_4 \cdot exper_i^2 + \beta_5 \cdot nwifeinc_i + \beta_6 \cdot kidslt6_i + \beta_7 \cdot kidsge6_i + u_i$$ $$~~~~~ (M.3)$$
...
El término $\varepsilon \sim N(0, \sigma^2)$ representa el error por lo tanto se estima el logarítmo de la desviación estándar del error.
...
#### Sección 1: Modelo Tobit: Estimación en R (Resultados) {.smaller}
 
 - Comparación de OLS y TOBIT ?
```{r}
#| warning: false
#| echo: false
#| label: tobit_outcomes
...
data("Mroz")
# Modelo OLS
modelo_ols <- lm(hours ~ age + educ + exper + I(exper^2) + 
                   nwifeinc + kidslt6 + kidsge6,
                 data = df_mroz)

# Modelo Tobit
modelo_tobit <- censReg(hours ~ age + educ + exper + I(exper^2) +
                          nwifeinc + kidslt6 + kidsge6,
                        left = 0,
                        data = df_mroz)

# Tabla comparativa
stargazer(modelo_ols, modelo_tobit,
          type = "text",
          title = "Comparación OLS vs Tobit",
...
- No podemos compararlos directamente y calculamos el factor de ajuste (incondicional) para este modelo.

- En la práctia se realiza con la $\bar{\mathbf{x}}$: 

$$\Phi\left(\frac{\bar{\mathbf{x_i}} \hat{\mathbf{\beta}}  }{\hat\sigma}\right)$$


#### Sección 1: Modelo Tobit: Estimación en R {.smaller}

1. Obtenemos promedio de los regresores $\bar{\mathbf{x}}$:

```{r}
#| warning: false
#| echo: true
#| label: tobit_outcomes2

X <- model.matrix(~ age + educ + exper + I(exper^2) + nwifeinc + kidslt6 + kidsge6, data = df_mroz)
X_mean <- colMeans(X)
X_mean["I(exper^2)"] <- X_mean["exper"]^2 ## O reemplaza la columna exper2 por el cuadrado de la media de exper

print(X_mean)
```
#### Sección 1: Modelo Tobit: Estimación en R {.smaller}

2. Obtenemos $\hat\sigma$ y $\hat{\mathbf{\beta}}$

```{r}
#| warning: false
#| echo: true
#| label: tobit_outcomes3
#Betas
beta <- coef(modelo_tobit)
beta_excluding_last <- beta[-length(beta)]
#sigma 
sigma <- exp(coef(modelo_tobit)['logSigma'])

```

3. Obtenemos el valor medio del predictor lineal del Tobit, dividido por la desviación estándar del término de error, lo que permite expresarlo en unidades de una Normal estándar:

$$z =\left(\frac{\bar{\mathbf{x_i}} \hat{\mathbf{\beta}}  }{\hat\sigma}\right)$$

```{r}
#| warning: false
#| echo: true
#| label: tobit_outcomes4

z_cond <- sum(X_mean * beta_excluding_last ) / sigma
...
4. Obtenemos el factor de ajuste: 

```{r}
#| warning: false
#| echo: true
#| label: tobit_outcomes5

#Factor incondicional (P(y>0))
#pnorm  calcula la función de distribución acumulada (CDF)
...
- La probabilidad estimada de que una mujer forme parte de la fuerza laboral, con los valores medios de las covariables, es de aproximadamente 0.645.


#### Sección 1: Modelo Tobit: Estimación en R {.smaller}

- Con el factor de ajuste estimado cálculamos los efectos marginales

```{r}
#| warning: false
#| echo: true
#| label: tobit_outcomes6

#Efectos parciales
margin_cond <- beta * fact
print(margin_cond)
```

#### Sección 1: Modelo Tobit: Interpretación de resultados {.smaller}

- Efectos parciales condicionales del modelo Tobit:

| Variable     | Coeficiente Ajustado | Interpretación |
|-------------|--------------------|----------------|
| Intercept   | 622.86             | Horas esperadas para una persona promedio con todas las covariables en 0. |
| age         | -35.10             | Cada año adicional de edad reduce ~35 horas esperadas, condicionado a y>0. |
| educ        | 52.03              | Cada año adicional de educación aumenta ~52 horas esperadas, condicionado a y>0. |
| exper       | 84.89              | Cada año de experiencia aumenta inicialmente las horas esperadas. |
| I(exper^2)  | -1.20              | Reduce el efecto de experiencia a medida que aumenta la experiencia (curva cuadrática). |
| nwifeinc    | -5.68              | Cada unidad de ingreso no laboral del hogar reduce ligeramente las horas esperadas. |
| kidslt6     | -576.86            | Tener un niño menor de 6 años reduce fuertemente las horas esperadas. |
| kidsge6     | -10.546            | Tener un niño entre 6 y 18 años reduce ligeramente las horas esperadas. |
...
-   Pero **solo observamos** $y_i$ si se cumple una condición:
    ...
    $$D_i = 1 $$ Por ejemplo: si la mujer trabaja, si el hogar se inscribe en el programa social, si la persona acepta declarar su ingreso

-   No todos los encuestados reportan ingreso, y los que lo hacen podrían tener **características distintas** (no comparables).
...
$$D_i^* = z_i \gamma + u_i, \quad u_i \sim N(0,1)$$ $$D_i = 
\begin{cases}
...
$$\log(\text{wage}_i) = x_i \beta + \rho \sigma_u \lambda(z_i \hat{\gamma}) + \varepsilon_i$$ - En este caso los regresores serán:

$$x_i = \text{educ}, \text{exper}, \text{exper}^2$$ - $\lambda(z_i \hat{\gamma})$: razón inverso de Mills
...
-   Heckman, J. J. (1976). The Common Structure of Statistical Models of Truncation, Sample Selection and Limited Dependent Variables. Econometrica, 47(1), 153–161.
