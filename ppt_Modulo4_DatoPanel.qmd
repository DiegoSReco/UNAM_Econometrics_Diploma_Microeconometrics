...
-   Micro Panel: Se compone por un gran número de observaciones (*N)* y períodos cortos de tiempo (*T*).
    ...
    -   Ejemplos: Datos de encuestas como Encuesta Nacional sobre Niveles de Vida de los Hogares (ENNViH) y ENOE del INEGI, National Longitudinal Surveys en EU, Canadian Labor Market

-   Macro Panel: Se compone por amplios períodos de tiempo y relativamente pocas observaciones.

    -   Ejemplos: Agregación de información de censos y de encuestas nivel estado, datos de países del Banco Mundial, OCDE, Euro Database, etc.
...
-   Se puede resolver uno de los problemas principales en la microeconometría e inferencia causal: **el problema de la variable omitida**
...
-   Relación de Esperanza de vida y PIB percapita: $\beta$ podría capturar tanto el efecto del PIB como el efecto de factores no observados correlacionados (infraestructura sanitaria, educación, estabilidad política), que además suelen ser similares dentro de cada continente,región, etc.
...
-   Donde **c** es una variable aleatoria que **no observamos** y es **constante en el tiempo**.

-   Tendrá un efecto parcial sobre el tiempo, y como no es es observado le llamaremos **efecto no observado**.

-   A nivel individual este tipo de variables son **preferencias personales, redes sociales, características como las habilidades cognitivas, la motivación, educación precoz, moral,etc.**
...
-   Las variables explicativas $\mathbf{x}$ están correlacionadas con el término de error $c$, la estimación por OLS con datos transversales sería sesgada e inconsistente.
...
-   Aunque el coeficiente $\delta$ es **fijo** para otras variables, los coeficientes $\theta_1$ y $\theta_2$ asociados a las dummies permiten capturar **efectos distintos en cada año**, es decir, permiten que el modelo refleje cambios por impacto del tiempo.
...
-   No hace distinción entre los dos tipos de variación: **entre individuos** y **a lo largo del tiempo**.
...
Estar afiliado a un sindicato se asocia con un incremento promedio de $(e^{0.1735} -1)*100 \approx 18.94%$ en el salario, manteniendo constantes las demás variables.
...
-   Interpretación: El coeficiente del log(gdpPercap) indica que un aumento del 1% en el PIB per cápita dentro de un mismo país está asociado con un aumento de 0.09769 años (alrededor de 35.6 días) en la esperanza de vida.
...
#### Modelos Lineales con Datos Panel Estáticos: Two-Way Fixed Effects VS FE (Interpretación) {.smaller}

**Distinción en la interpretación FE y TWFE**

- Recordemos que el modelo FE, mediante la transformación **"within"**, elimina toda la variación entre individuos. **Solo utiliza la variación dentro de cada individuo a lo largo del tiempo.**

>"Cuando el PIB per cápita de un país aumenta en un 1% a lo largo del tiempo, la esperanza de vida de ese mismo país aumenta en aproximadamente 35.7 días."

- En TWFE controla por factores que cambian en el tiempo, pero que afectan a **todas** las unidades de análisis por igual.

>"Cuando el PIB per cápita de un país aumenta en un 1%, la esperanza de vida de ese mismo país aumenta en aproximadamente 5.3 días (0.0145 años), después de descontar los factores no observados que son constantes para el país y las tendencias o shocks que afectan a todos los países por igual en ese período de tiempo."


#### Modelos Lineales con Datos Panel Estáticos: Two-Way Fixed Effects VS FE (Interpretación) {.smaller}

| Elemento de Control              | Lo que significa para el Coeficiente de 1.450 |
|----------------------------------|------------------------------------------------|
| **Efectos Fijos Individuales (αᵢ)** | Este efecto de 5.3 días ya no se debe a características constantes del país (como ser una nación insular, su cultura o su calidad institucional inmutable). |
| **Efectos Fijos de Tiempo (δₜ)**    | Este efecto de 5.3 días ya no se debe a avances médicos globales, pandemias o tendencias mundiales que afectan a todos los países por igual en ese año. |
...
Ser miembro de un sindicato se asocia con un aumento promedio de 8.1% en el salario  de un mismo individuo a lo largo del tiempo.
...
-   En FE el objetivo es eliminar $c_i$ con el **método demeaned** (extracción de la media), ya que pensamos que los $c_i$ están correlacionado con $x_{it}$.

Discusión: **¿Tratar como variable aleatoría o como un parámetro el cual tenemos que estimar?**
...
$$ \Omega \equiv \mathbb{E}(\textbf{v}_i \textbf{v}_i')$$ - Es decir la matriz de varianzas y covarianza
...
$$ \hat\sigma_v^2 = \frac{1}{NT-K} \sum_{i=1}^N \sum_{t=1}^T \hat{\hat{v}} _{it}^2$$ Donde: $NT-K$ es la corrección por grados de libertad

#### Modelos Lineales con Datos Panel:Random Effects (Supuestos) {.smaller}

Para obtener un estimador de $\sigma^2_{c}$ recordamos que $\sigma^2_{c} = E(v_{it}v_{is})$ :

$$\hat\sigma_c^2 = \frac{1}{[NT(T-1)/2-K]} \sum_{i=1}^N \sum_{t=1}^{T-1} \sum_{s=t+1}^T \hat{\hat{v}}_{it} \hat{\hat{v}}_{is}$$ Donde: $NT(T-1)/2-K$ es la corrección por grados de libertad
